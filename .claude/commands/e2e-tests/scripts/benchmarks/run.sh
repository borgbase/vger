...
# Reproducible benchmark harness for vger vs restic vs rustic on a local repo.
#
...
sudo -n mkdir -p "$VGER_REPO" "$RESTIC_REPO" "$RUSTIC_REPO"
sudo -n chown -R "$USER:$USER" "$VGER_REPO" "$RESTIC_REPO" "$RUSTIC_REPO"

rm -rf "$VGER_REPO"/* "$RESTIC_REPO"/* "$RUSTIC_REPO"/*
...
mkdir -p "$RESTORE_VGER" "$RESTORE_RESTIC" "$RESTORE_RUSTIC"
...
# One-shot profiling runs (off by default). Enable by exporting PROFILE=1.
if [[ "${PROFILE:-0}" == "1" ]]; then
...
# Repo size stats
du -sh "$VGER_REPO" "$RESTIC_REPO" "$RUSTIC_REPO" >"$OUT_ROOT/repo-sizes.txt"
...
- repo-sizes.txt
- vger.info.txt / restic.stats.txt / rustic.stats.txt
- logs/ (hyperfine stdout/stderr)
- profile.<op>/ (only if PROFILE=1)
EOF
