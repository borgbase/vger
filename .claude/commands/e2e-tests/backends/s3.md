...
1. Delete repo from previous runs (best effort):
   ```bash
   vger -c <config> delete -R s3 --yes-delete-this-repo || true
   ```
2. Initialize repo:
   ```bash
   vger -c <config> init -R s3
   ```
3. Run backup:
   ```bash
   vger -c <config> backup -R s3 -l remote-corpus ~/corpus-remote
   ```
4. Confirm snapshot:
   ```bash
   vger -c <config> list -R s3 --last 3
   ```
5. Capture latest snapshot ID.
6. Restore to empty temp directory:
   ```bash
   vger -c <config> restore -R s3 --dest <restore_dir> <snapshot_id>
   ```
7. Integrity check:
   ```bash
   vger -c <config> check -R s3
   ```
8. Delete the tested snapshot:
   ```bash
   vger -c <config> snapshot -R s3 delete <snapshot_id>
   ```
9. Compact repository packs:
   ```bash
   vger -c <config> compact -R s3
   ```
