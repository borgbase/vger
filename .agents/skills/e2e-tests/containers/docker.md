---
name: docker
description: "Validate vger workflows with Docker: static volumes, downtime hooks, DB dumps"
---
...
Validate vger workflows for Docker-backed sources:
...
3. Use `sudo vger ...` when backing up paths under `/var/lib/docker/volumes/...`
...
1. Create a Docker volume and seed it with test files:
   ```bash
   sudo docker volume create vger-static-vol
   ```
2. Resolve host volume path:
   ```bash
   sudo docker volume inspect vger-static-vol --format '{{ .Mountpoint }}'
   ```
