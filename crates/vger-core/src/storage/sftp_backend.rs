...
impl RetryError {
    fn transient(err: VgerError) -> Self {
        Self {
            err,
            retryable: true,
        }
    ...
    fn permanent(err: VgerError) -> Self {
        Self {
            err,
            retryable: false,
        }
    ...
    fn acquire_conn(&self) -> RetryResult<ConnGuard<'_>> {
        ...
        loop {
            ...
            if remaining.is_zero() {
                return Err(RetryError::transient(VgerError::Other(
    ...
    async fn connect(params: &SftpConnectParams) -> RetryResult<SftpConn> {
        ...
        let mut session =
            tokio::time::timeout(CONNECT_TIMEOUT, client::connect(config, addr, handler))
                .await
                .map_err(|_| {
                    RetryError::transient(VgerError::Other(format!(
...
fn load_key(explicit: &Option<PathBuf>) -> RetryResult<PrivateKey> {
    if let Some(path) = explicit {
        return load_secret_key(path, None).map_err(|e| {
            RetryError::permanent(VgerError::Other(format!(
                "load SSH key {}: {e}",
                path.display()
            )))
        });
    ...
    Err(RetryError::permanent(VgerError::Other(
...
fn ssh_retry_error(op: &str, host: &str, port: u16, e: russh::Error) -> RetryError {
    let err = VgerError::Other(format!("SSH {op} {host}:{port}: {e}"));
...
fn sftp_retry_error(op: &str, path: &str, e: russh_sftp::client::error::Error) -> RetryError {
    let err = VgerError::Other(format!("SFTP {op} '{path}': {e}"));
...
mod tests {
    ...
    fn test_normalize_root() {
        ...
        assert_eq!(normalize_root("/"), "/");
        assert_eq!(normalize_root("backups/vger"), "/backups/vger");
        assert_eq!(normalize_root("/backups/vger/"), "/backups/vger");
    }
